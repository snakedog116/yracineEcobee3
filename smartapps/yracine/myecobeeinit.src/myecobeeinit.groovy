...
def otherSettings() {
	dynamicPage(name: "otherSettings", title: "Other Settings", install: true, uninstall: false) {
		section("Polling at which interval in minutes (range=[10..59],default=20 min.)?") {
...
def takeAction() {
	...
	Integer delay = givenInterval ?: 20 // By default, the poll is done every 20 min.
	state?.poll["last"] = now()
	...
	if (handleException) {    
		if ((exceptionCheck) && (exceptionCheck.contains("Unauthorized")) && (state?.exceptionCount>=MAX_EXCEPTION_COUNT)) {
			// need to authenticate again    
			atomicState.authToken=null                    
			state?.oauthTokenProvided=false
			msg="$exceptionCheck after ${state?.exceptionCount} errors, press on 'ecobee' and re-login..." 
			send "MyEcobeeInit> ${msg}"
			log.error msg
		} else if (state?.exceptionCount>=MAX_EXCEPTION_COUNT) {
...
def swapToken() {
	...
	def authexptime = new Date((now() + (atomicState.expiresIn * 1000))).getTime()
