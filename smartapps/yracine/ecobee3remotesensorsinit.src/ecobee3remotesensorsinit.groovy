...
private def createMotionSensors() {
	...
	def devices = motionSensors.collect {dni->
		...
		if (!d) {
			...
			def labelName = "${tstatName}:${getMotionSensorChildName()} ${sensorName}"
...
private def createTempSensors() {
	...
	def devices = tempSensors.collect {dni->
		...
		if (!d) {
			...
			def labelName = "${tstatName}:${getTempSensorChildName()} ${sensorName}"
...
def takeAction() {
	...
	try {        
    	...
    	state.exceptionCount=state.exceptionCount+1    
		log.error "takeAction>exception $e while trying to generateRemoteSensorEvents on device $d, exceptionCount= ${state?.exceptionCount}" 
	}
	if (handleException) {            
